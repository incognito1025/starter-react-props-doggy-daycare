...
function Main( {dogs}) {
  ...
  return (
    <main>
      ...
      <ul>
        {dogs.map((dog) => { //dogs before dogs.map is passed as a prop to the `Main` component
          return (
            //key={dog.name}: The key attribute helps React identify which items have changed, are added, or are removed. It needs a unique value for each item. Here, the dog's name is used, which assumes names are unique. If names are not unique, it's better to use a different unique identifier.
            <li key={dog.name}>
              <span>{dog.name}:</span> {dog.notes}
              <span>&nbsp; Grade:</span> {dog.grade}
              {/* &nbsp;: This is an HTML entity for a non-breaking space, which adds a space between the notes and the "Grade" label. */}
            </li>
          );
        })}
      </ul>
